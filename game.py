...
def beach_game(window_name):
    ...
    def random_garbage_pos(min_x, max_x, garbage_h, garbage_w):
        x = random.randint(210, max_x - garbage_w)
    ...
    frame_counter = 0
    is_moving = False
    facing_right = True  # 角色面向，False為左，True為右
    ...
    while True:
            ...
            if is_moving:
                ...
                if frame_counter >= 3:  # 控制動畫速度
                    ...
                    current_frame = (current_frame + 1) % (
                        len(std_frames_right) if facing_right else len(std_frames_left))
                    if facing_right:
        ...
        if in_recycle_area:
            ...
            # 繪製回收區背景
            if current_bg in recycle_backgrounds and recycle_backgrounds[current_bg] is not None and \
                    recycle_backgrounds[current_bg].shape[:2] == (window_height, window_width):
            ...
            if collected_garbage:
                ...
                for item_info in garbage_matrix:
                    if item_info["type"] == top_garbage.garbage_type and garbage_images.get(
                            item_info["file"]) is top_garbage.image:
            ...
            # 顯示回收區提示文字
            if collected_garbage:
                cv2.putText(view_frame, "Drag garbage to the correct bin", (window_width // 2 - 200, 100), font, 1,
                            (0, 0, 0), 2)
            else:
            ...
            for item in garbage_items:
                if item.visible:
                    ...
                    if -item_w < item_x < window_width:
                        ...
                        character_lower_quarter_h = std_h - int(std_h * 9 / 10)  # 下四分之一的高度
                        ...
                        if x_end > x_start and y_end > y_start:
                            ...
                            if item_image.shape[2] == 4:  # 如果有 Alpha 通道
                                ...
                                view_frame[y_start:y_end, x_start:x_end] = item_image[
                                                                           img_start_y:img_start_y + (y_end - y_start),
                                                                           img_start_x:img_start_x + (x_end - x_start),
                                                                           :3]

            # 檢查遊戲是否結束 (海灘上沒有可見垃圾且沒有已收集垃圾)
            if len([item for item in garbage_items if
                    item.visible]) == 0 and not collected_garbage and not in_recycle_area:
                game_over = True
...
def main():
    ...
    for key, path in recycle_bg_paths.items():
        if not os.path.exists(path):
            ...
            cv2.putText(temp_img, f"{key.replace('_open', '').replace('_close', '')} Bin Placeholder", (400, 400),
